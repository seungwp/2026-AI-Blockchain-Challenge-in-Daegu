...
class LlmAdviceServiceTest {
    ...
    void 입력에_있는_숫자만_쓰면_통과한다() {
        ...
        assertFalse(LlmAdviceService.hasInventedNumber(FACTS, text));
    }

    @Test
    void 입력에_없는_숫자를_지어내면_걸러진다() {
        String text = "이번 주 매출이 15% 오를 것으로 예상됩니다.";
        assertTrue(LlmAdviceService.hasInventedNumber(FACTS, text));
    }
    ...
    void 숫자가_아예_없으면_항상_통과한다() {
        assertFalse(LlmAdviceService.hasInventedNumber(FACTS, "특별한 조건 없이 평소처럼 운영하시면 됩니다."));
    }
}
