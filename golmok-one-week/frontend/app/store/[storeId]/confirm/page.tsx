...
export default function StoreConfirmPage() {
  ...
  const suggestion = suggestMenus(store?.name ?? null, store?.category ?? null);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      classifyMenu(menu, store.category ?? undefined).then((result) => {
        if (!active || readDraft().mainMenu.trim() !== menu) return;
        saveDraft({ classification: result, menuCategory: result.menuCategory, classifiedMenu: menu });
        setError("");
      }).catch(() => {
        if (active) setError("메뉴를 확인하지 못했어요. 다시 시도해주세요.");
      });
    }, 400);
  ...
  return <OnboardingShell step={3} back="/search" eyebrow="어떤 메뉴를 파세요?" title="대표 메뉴를 입력해주세요"
    ...
    {!store ? <p className={styles.status} role="status">가게 정보를 확인하고 있어요…</p> : <>
      <form onSubmit={form.handleSubmit(confirmMenu)} noValidate>
        ...
        <div className={styles.menuEntry}>
          <input id="mainMenu" className={styles.menuInput} placeholder={suggestion.menus[0] ?? "메뉴 이름 입력"} enterKeyHint="done"
      ...
      {resolved && draft.classification && <section className={styles.classificationCard} aria-label="메뉴 분류 결과">
        <p className={styles.classificationLabel}>자동 분류 결과</p>
        <div className={styles.classificationResult}><strong>{menu}</strong><span>→</span><b>{draft.menuCategory}</b></div>
        <p>‘{draft.menuCategory}’ 기준으로 식자재 가격과 이번 주 준비 거리를 챙겨드려요.</p>
        <details className={styles.categoryPicker}><summary>카테고리가 다른가요? <span>직접 선택하기⌄</span></summary>
          <div>{Object.keys(menuSuggestions).map((category) => <button type="button" key={category}
            className={draft.menuCategory === category ? styles.categorySelected : undefined}
            onClick={() => saveDraft({ menuCategory: category as MenuCategory })}>{category}</button>)}</div>
        </details>
      </section>}
